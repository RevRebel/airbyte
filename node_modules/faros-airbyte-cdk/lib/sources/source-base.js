...
class AirbyteSourceBase extends source_1.AirbyteSource {
    ...
    async *read(config, catalog, state) {
        ...
        for (const configuredStream of catalog.streams) {
            ...
            try {
                ...
                this.logger.error(`Encountered an error while reading stream ${this.name}: ${(_a = e.message) !== null && _a !== void 0 ? _a : JSON.stringify(e)}`);
                throw e;
            }
        }
    ...
    async *readIncremental(streamInstance, configuredStream, connectorState) {
        ...
        if (streamState) {
            this.logger.info(`Setting state of ${streamName} stream to ${JSON.stringify(streamState)}`);
        }
        ...
        for await (const slice of slices) {
            ...
            for await (const recordData of records) {
                ...
                if (checkpointInterval && recordCounter % checkpointInterval === 0) {
                    yield this.checkpointState(streamName, streamState, connectorState, recordCounter);
                }
            }
            yield this.checkpointState(streamName, streamState, connectorState, recordCounter);
        }
    ...
    checkpointState(streamName, streamState, connectorState, recordCounter) {
        this.logger.info(`Setting state of ${streamName} stream to ${JSON.stringify(streamState)}`);
